...
export class AdsService {
  ...
  deletePublicityUrl =
    'http://localhost:9091/SpringMVC/servlet/remove-publicity/';
  assignImageToPublicityUrl =
    'http://localhost:9091/SpringMVC/servlet/affect-image-to-publicity/';
  getPublicityByIdUrl =
    'http://localhost:9091/SpringMVC/servlet/get-by-id-publicity/';
  getPublicitiesByNameurl =
    'http://localhost:9091/SpringMVC/servlet/get-by-Name-all-publicities-by-name/';
  ...
  updatePublicity(id: number, publicity: Publicity) {
    return this.httpPublicity.put<Publicity>(
      this.updatePublicityUrl + id,
      publicity
    );
  }
  ...
  assignImageToPublicity(idPublicity: number, idImage: number) {
    return this.httpPublicity.put(
      this.assignImageToPublicityUrl + idPublicity,
      idImage
    );
  }

  getPublicityById(publicityId: number) {
    return this.httpPublicity.get<Publicity>(
      this.getPublicityByIdUrl + publicityId
    );
  }

  getPublicitiesByName(namePublicity: string) {
    return this.httpPublicity.get<Publicity[]>(
      this.getPublicitiesByNameurl + namePublicity
    );
  }
}
